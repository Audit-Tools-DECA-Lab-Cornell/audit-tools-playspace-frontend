 ...
 * reader sets an override. Override keys are `scores.by_domain` keys — the same
 * normalized domain keys `getQuestionDomainKeys` returns — not section keys.
 */
...
export function buildQuestionLookup(
	instrument: PlayspaceInstrumentInput
): Readonly<Record<string, InstrumentQuestion>> {
...
export function reportFilterExcludesConstruct(filter: ReportResultFilter): boolean {
	return (
		!filter.overall.playValue ||
		!filter.overall.usability ||
		Object.values(filter.domainOverrides).some(selection => !selection.playValue || !selection.usability)
	);
}
